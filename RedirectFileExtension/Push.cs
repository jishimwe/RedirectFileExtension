...
namespace RedirectFileExtension
{
	...
	internal sealed class Push
	{
		...
		private Push(AsyncPackage package, OleMenuCommandService commandService)
		{
		...
		{
			// Switch to the main thread - the call to AddCommand in Push's constructor requires
			// the UI thread.
			await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(package.DisposalToken);

			OleMenuCommandService commandService = await package.GetServiceAsync(typeof(IMenuCommandService)) as OleMenuCommandService;
			Instance = new Push(package, commandService);
		}
		...
		private void Execute(object sender, EventArgs e)
		{
			...
			string message = "Push not executed";
			string title = "Push";
			...
			MyForm form = new MyForm(data, "Push");
